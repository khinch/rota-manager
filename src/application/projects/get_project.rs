use color_eyre::eyre::eyre;

use crate::{
    app_state::ProjectStoreType,
    application::projects::{check_project_access, ApplicationError},
    domain::{Project, ProjectId, ProjectStoreError, UserId},
};

pub async fn get_project(
    project_store: &ProjectStoreType,
    user_id: UserId,
    project_id: ProjectId,
) -> Result<Project, ApplicationError> {
    check_project_access(&project_store, &user_id, &project_id).await?;

    let project = project_store
        .write()
        .await
        .get_project(&project_id)
        .await
        .map_err(|e| match e {
            ProjectStoreError::ProjectIDNotFound => {
                ApplicationError::ProjectIDNotFound(project_id.clone())
            }
            _ => ApplicationError::UnexpectedError(eyre!(e)),
        })?;

    Ok(project)
}

#[cfg(test)]
mod tests {
    use crate::{
        app_state::ProjectStoreType,
        application::projects::{get_project, ApplicationError},
        data_stores::test_utils::*,
        domain::{ProjectId, ProjectName, UserId},
    };
    use std::sync::Arc;
    use tokio::sync::RwLock;

    fn init_store() -> ProjectStoreType {
        let store: ProjectStoreType =
            Arc::new(RwLock::new(test_init_hashmap_store()));
        store
    }

    #[tokio::test]
    async fn get_empty_project_success() {
        let store = init_store();

        let user_id = UserId::parse(EMPTY_PROJECT_USER_ID).unwrap();
        let project_id = ProjectId::parse(EMPTY_PROJECT_PROJECT_ID).unwrap();

        let project = get_project(&store, user_id, project_id.clone()).await;
        assert!(project.is_ok());

        let project = project.unwrap();
        assert_eq!(
            project.project_name,
            ProjectName::parse(EMPTY_PROJECT_PROJECT_NAME).unwrap()
        );
        assert_eq!(project.project_id, project_id);
        assert!(project.members.is_empty());
    }

    #[tokio::test]
    async fn non_existent_project_ids_should_return_not_found() {
        let store = init_store();

        let user_id = UserId::parse(U1_USER_ID).unwrap();
        let project_id = ProjectId::default();

        let result = get_project(&store, user_id, project_id.clone()).await;
        assert!(result.is_err_and(|e| {
            e == ApplicationError::ProjectIDNotFound(project_id)
        }));
    }

    #[tokio::test]
    async fn project_ids_belonging_to_another_user_should_return_not_found() {
        let store = init_store();

        let test_params = [
            (U1_USER_ID, U2_P1_PROJECT_ID),
            (U1_USER_ID, U2_P2_PROJECT_ID),
            (U2_USER_ID, U1_P1_PROJECT_ID),
            (EMPTY_PROJECT_USER_ID, U1_P1_PROJECT_ID),
        ];

        for params in test_params {
            let user_id = UserId::parse(params.0).unwrap();
            let project_id = ProjectId::parse(params.1).unwrap();

            assert!(get_project(&store, user_id, project_id.clone())
                .await
                .is_err_and(
                    |e| e == ApplicationError::ProjectIDNotFound(project_id)
                ));
        }
    }

    #[tokio::test]
    async fn get_project_without_shifts_success() {
        let store = init_store();

        let user_id = UserId::parse(U2_USER_ID).unwrap();
        let project_id = ProjectId::parse(U2_P1_PROJECT_ID).unwrap();

        let project = get_project(&store, user_id, project_id.clone())
            .await
            .expect("Failed to fetch project");

        assert_eq!(
            &project.project_name,
            &ProjectName::parse(U2_P1_PROJECT_NAME).unwrap()
        );

        assert_eq!(
            &project.project_id,
            &ProjectId::parse(U2_P1_PROJECT_ID).unwrap()
        );

        assert_eq!(project.members.len(), 2);
    }

    #[tokio::test]
    async fn get_project_with_shifts_success() {
        let store = init_store();

        let user_id = UserId::parse(U2_USER_ID).unwrap();
        let project_id = ProjectId::parse(U2_P2_PROJECT_ID).unwrap();

        let project = get_project(&store, user_id, project_id.clone())
            .await
            .expect("Failed to fetch project");

        assert_eq!(
            &project.project_name,
            &ProjectName::parse(U2_P2_PROJECT_NAME).unwrap()
        );

        assert_eq!(
            &project.project_id,
            &ProjectId::parse(U2_P2_PROJECT_ID).unwrap()
        );

        assert_eq!(project.members.len(), 2);
    }
}
